...
class LibraryBook(models.Model):
    ...
    primary_author = fields.Char(string='Primary Author', compute='_compute_primary_author', store=True, index=True)
    ...
    def create(self, vals):
        # Ensure the product has a name when creating
        if vals.get('title') and not vals.get('product_id'):
            # Prepare product values
            product_vals = {
                'name': vals.get('title'),
                'type': 'consu',  # Consumable product
                'categ_id': self.env.ref('product.product_category_1').id,  # Default category
    ...
    def write(self, vals):
        # Update product name when book title changes
        if 'title' in vals:
            for book in self:
                if book.product_id:
